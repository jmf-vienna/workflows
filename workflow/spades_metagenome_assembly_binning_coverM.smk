...
rule complete:
    input:
        expand("results/{name}_spades/final_bins/dereplicated_genomes/final_QC_cov.xlsx", name=NAMES)
...
rule SPAdes_assembly:
    ...
    conda:
        "envs/spades.yaml"
    resources: mem_mb=500000, runtime="4d", partition="basic"
    log: "log/SPADES_{sample}.log"
    shell:
        """
        spades.py -t {threads} -m 500 -k 21,31,41,51,61,71,81,91,101,111,121 --meta --tmp-dir $TMPDIR --pe-12 1 {input} -o results/{wildcards.sample}_spades 2> {log}
...
rule metabat_nocov:
    ...
    conda:
        "envs/metabat.yaml"
    resources: mem_mb=50000, runtime="2d", partition="basic"
...
rule mapping_prep:
    ...
    resources: mem_mb=100000, runtime="4d", partition="basic"
...
rule gtdb_binning:
    ...
    conda:
        "envs/gtdbtk.yaml"
    resources: mem_mb=250000, runtime="2d", partition="basic"
    log: "log/gtdbtk_{sample}.log"
    shell:
        """
        gtdbtk classify_wf --extension fa --cpus {threads} --genome_dir results/{wildcards.sample}_spades/final_bins/dereplicated_genomes --out_dir results/{wildcards.sample}_spades/final_bins/dereplicated_genomes/gtdb_classification 2> {log}
...
rule coverM:
    input:
        "results/{sample}_spades/final_bins/dereplicated_genomes/final_QC.xlsx"
    output:
        "results/{sample}_spades/final_bins/dereplicated_genomes/coverM_relative_output.tsv",
        "results/{sample}_spades/final_bins/dereplicated_genomes/coverM_mean_output.tsv"
    threads: 16
    conda:
        "envs/coverm.yaml"
    resources: mem_mb=100000, runtime="1d", partition="basic"
    log:
        "log/coverm_{sample}.log"
    shell:
        """
        files=$(ls data/interleave_DNA/* | tr '\n' ' ')

        coverm genome -d results/{wildcards.sample}_spades/final_bins/dereplicated_genomes/ --interleaved $files -x fa --methods relative_abundance -o {output[0]} -t 16
        coverm genome -d results/{wildcards.sample}_spades/final_bins/dereplicated_genomes/ --interleaved $files -x fa --methods mean -o {output[1]} -t 16
        """
...
rule merge_cov:
    input:
        ...
        "results/{sample}_spades/final_bins/dereplicated_genomes/coverM_mean_output.tsv"
    output:
        "results/{sample}_spades/final_bins/dereplicated_genomes/final_QC_cov.xlsx"
    threads: 1
    conda:
        "envs/python3_modules.yaml"
    resources: mem_mb=10000, runtime="1h", partition="basic"
    shell:
        """
        python3 workflow/scripts/final_coverM_merge.py {input[0]} {input[1]} {input[2]} {output[0]}
        """

...
rule mapping_prep:
	...
	shell:
                ...
                for f in data/interleave/*.interleave.fastq.gz; do g=${{f##*/}}; bbmap.sh -Xmx50g threads={threads} ref={input} nodisk minid=0.98 in=$f out=results/{wildcards.sample}/bams/${{g%%.interleave.fastq.gz}}.bam; done 2> {log}
...
rule metabat_cov:
	...
	shell:
		...
		metabat2 -i {input[0]} -o results/{wildcards.sample}_spades/metabat_cov/metabat_cov -m 1500 -t 8 2> {log}
		"""
